...
$sql="select * from patient p,category c where c.catid=p.catid and p.uemail='$email' and p.status=1";
...
{   
    ...
    $sql5="UPDATE `patient` SET `status`=0 WHERE uemail='$email'";
    $result5=mysqli_query($con,$sql5);
    $sql5="DELETE FROM `allocate` WHERE pid=$pid";
    $result5=mysqli_query($con,$sql5);
    if($result5==TRUE)
    {
    echo"<script>
    alert('Patient Deleted Successfull');
    window.location='delete_patient.php';
    </script>";
    }
    else
    {
    echo"<script>alert('Patient Deletion failed');
    window.location='delete_patient.php';
    </script>";
    }
}
...
?>
        ...
        <div class="col-md-12 form-group">
          ...
          <div class="text-center"><button class="btn bg-danger appointment-btn scrollto" type="submit" name="delete">Delete</button></div>
        </form>
